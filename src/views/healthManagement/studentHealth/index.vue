...
<script lang="ts" setup>
  ...
  import {
    getStudentHealthPage,
    deleteStudentHealthById,
  } from '@/api/admin/healthManagement/student.ts'
  ...
  const handleDeleteListClick = () => {
    ElMessageBox.confirm('请确认要批量删除该组数据 ?', '健康管理', {
      ...
      selectedRows.value.map(async (item) => {
        // 调用删除接口
        await deleteStudentHealthById(item.userId)
      })
      // 刷新数据
      freshData()
      // 确认删除
  ...
  const handleDeleteOneClick = (row) => {
    ElMessageBox.confirm('请确认要删除该数据 ?', '健康管理', {
      ...
      // 确认删除
      await deleteStudentHealthById(row.userId)
      // 刷新数据
      freshData()
      ElMessage.success('删除成功')
    })
  ...
  const freshData = () => {
    ...
    setTimeout(async () => {
      ...
      const resp = await getStudentHealthPage(current, size, id, name)
  ...
  const handleExportExcel = async () => {
    // 获取全部数据
    const resp = await getStudentHealthPage(1, total.value)
    console.log(resp)
      ...
      <div class="fuzzy_query">
        <div class="top">
          <el-form :model="fuzzyFormData" label-width="auto" class="form">
            <el-form-item label="学号" width="120" class="form-item">
