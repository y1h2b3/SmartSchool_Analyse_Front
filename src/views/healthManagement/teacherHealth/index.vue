...
<script lang="ts" setup>
  ...
  import {
    getTeacherHealthPage,
    deleteTeacherHealthById,
  } from '@/api/admin/healthManagement/teacher.ts'
  ...
  const handleDeleteListClick = () => {
    ElMessageBox.confirm('请确认要批量删除该组数据 ?', '健康管理', {
      ...
      selectedRows.value.map(async (item) => {
        // 调用删除接口
        await deleteTeacherHealthById(item.userId)
      })
      // 刷新数据
      freshData()
      // 确认删除
  ...
  const handleDeleteOneClick = (row) => {
    ElMessageBox.confirm('请确认要删除该数据 ?', '健康管理', {
      ...
      // 确认删除
      await deleteTeacherHealthById(row.userId)
      // 刷新数据
      freshData()
      ElMessage.success('删除成功')
    })
  ...
  const freshData = () => {
    ...
    setTimeout(async () => {
      ...
      const resp = await getTeacherHealthPage(current, size, id, name)
  ...
  const handleExportExcel = async () => {
    // 获取全部数据
    const resp = await getTeacherHealthPage(1, total.value)
      ...
      <div class="fuzzy_query">
        <div class="top">
          <el-form :model="fuzzyFormData" label-width="auto" class="form">
            <el-form-item label="教师工号" width="120" class="form-item">
