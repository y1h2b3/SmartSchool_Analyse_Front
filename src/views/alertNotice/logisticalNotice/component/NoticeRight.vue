...
<script lang="ts" setup>
  ...
  import {
    getAllLogisticalHealthWarningNotifications,
    getPageLogisticalHealthWarningNotifications,
    deleteLogisticalHealthWarningNotifications,
  } from '@/api/admin/alertNotice/logistical.ts'
  ...
  const handleDeleteListClick = () => {
    ElMessageBox.confirm('请确认要批量删除该组数据 ?', '预警管理', {
      ...
      selectedRows.value.map(async (item) => {
        // 调用删除接口
        await deleteLogisticalHealthWarningNotifications(item.userId)
      })
      // 刷新数据
      freshData()
  ...
  const handleDeleteOneClick = (index: number) => {
    ElMessageBox.confirm('请确认要删除该数据 ?', '预警管理', {
      ...
      await deleteLogisticalHealthWarningNotifications(id)
      freshData()
      ElMessage.success('删除成功')
    })
  }
  ...
  const freshData = () => {
    ...
    setTimeout(async () => {
      ...
      const resp = await getPageLogisticalHealthWarningNotifications(
        current,
        size,
        id,
        level,
  ...
  const handleExportExcel = async () => {
    // 获取全部数据
    const resp = await getAllLogisticalHealthWarningNotifications()
  ...
  <div class="right-content">
    ...
    <el-scrollbar class="top" :style="{ height: `${isTwoFuzzy ? 120 : 70}px` }">
      <el-form class="el-form" :model="fuzzyFormData">
        <el-form-item label="后勤工号" label-width="70" class="el-form-item" prop="id">
          <el-input v-model="fuzzyFormData.id" placeholder="请输入后勤工号" style="width: 150px" />
        </el-form-item>
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="userId" label="后勤学号" width="140"></el-table-column>
        <el-table-column prop="userName" label="后勤姓名" width="80"></el-table-column>
        ...
        <el-table-column prop="readingValue" label="检测结果" width="160"></el-table-column>
