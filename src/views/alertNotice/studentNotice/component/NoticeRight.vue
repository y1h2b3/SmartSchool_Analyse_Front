...
<script lang="ts" setup>
  ...
  import {
    ...
    getPageStudentHealthWarningNotifications,
    deleteStudentHealthWarningNotifications,
  ...
  const handleDeleteListClick = () => {
    ElMessageBox.confirm('请确认要批量删除该组数据 ?', '预警管理', {
      ...
      selectedRows.value.map(async (item) => {
        // 调用删除接口
        await deleteStudentHealthWarningNotifications(item.userId)
      })
      // 刷新数据
      freshData()
  ...
  const handleDeleteOneClick = (index: number) => {
    ElMessageBox.confirm('请确认要删除该数据 ?', '预警管理', {
      ...
      await deleteStudentHealthWarningNotifications(id)
      freshData()
      ElMessage.success('删除成功')
    })
  }
  ...
  const freshData = () => {
    ...
    setTimeout(async () => {
      ...
      const resp = await getPageStudentHealthWarningNotifications(
        current,
        size,
        id,
        level,
  ...
  const handleExportExcel = async () => {
    // 获取全部数据
    const resp = await getAllStudentHealthWarningNotifications()
  ...
  <div class="right-content">
    ...
    <el-scrollbar class="top" :style="{ height: `${isTwoFuzzy ? 120 : 70}px` }">
      <el-form class="el-form" :model="fuzzyFormData">
        <el-form-item label="学生学号" label-width="70" class="el-form-item" prop="id">
          <el-input v-model="fuzzyFormData.id" placeholder="请输入学生学号" style="width: 150px" />
        </el-form-item>
    ...
    <div class="table-container">
      ...
      >
        ...
        <el-table-column prop="userName" label="学生姓名" width="80"></el-table-column>
        ...
        <el-table-column prop="userPost" label="班级" width="100"></el-table-column>
