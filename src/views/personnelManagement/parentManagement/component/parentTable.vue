<script lang="ts" setup>
  ...
  import ParentInsertDialog from './parentInsertDialog.vue'
  import ParentUpdateDialog from './parentUpdateDialog.vue'
  ...
  import {
    ...
    deleteParentInfo,
    updateParentInfo,
  } from '@/api/admin/personnelManagement/parent.ts'
  const instance = getCurrentInstance()
  const props = defineProps(['currentParentNode', 'currentChildrenNode', 'allFreshData'])
  const tableList = ref([]) // 当前页数据
  const columnList = ref([]) // 导出当前页数据
  ...
  const column = [
    { name: 'order', label: '序号' },
    { name: 'parentId', label: '监护人ID' },
    { name: 'parentName', label: '监护人姓名' },
    { name: 'sex', label: '性别' },
    { name: 'studentId', label: '儿女学号' },
    { name: 'studentName', label: '儿女姓名' },
    { name: 'grade', label: '年级' },
    { name: 'clazz', label: '班级' },
  ...
  const hanldeUpdateAllFreshData = () => {
    instance.emit('updateAllFreshData', !props.allFreshData.value)
  }
  // 刷新数据
  const flushData = (index: number) => {
    ...
    setTimeout(async () => {
      // 获取新的数据
      const clazz = index === 0 ? '' : props.currentChildrenNode
      const pName = searchForm.pName
      const sName = searchForm.sName
      const resp = await getPageParentInfo(currentPage.value, pageSize.value, clazz, pName, sName)
  ...
  const handleBatchDelete = async () => {
    ...
    selectedRows.value.map(async (item) => {
      // 调用删除接口
      await deleteParentInfo(item.parentId)
    })
    // 刷新数据
  ...
  const handleDelete = async (row) => {
    ...
    // 调用删除接口
    await deleteParentInfo(row.parentId)
    // 刷新数据
    instance.emit('updateAllFreshData', !props.allFreshData.value)
    flushData(currentFresh.value)
    ElMessage.success('删除成功')
  ...
  const handleExportExcel = async () => {
    // 获取当前页数据
    const clazz = props.currentChildrenNode
    const pName = searchForm.pName
    const sName = searchForm.sName
    const pageResp = await getPageParentInfo(currentPage.value, pageSize.value, clazz, pName, sName)
    ...
    for (let i = 0; i < columnList.value.length; i++) {
      ...
      columnList.value[i].status = columnList.value[i].status === 0 ? '禁用' : '正常'
    ...
    for (let i = 0; i < tableAllList.value.length; i++) {
      ...
      tableAllList.value[i].status = tableAllList.value[i].status === 1 ? '正常' : '禁用'
  ...
  <div class="content">
    ...
    <el-scrollbar class="top">
      <el-form class="el-form" :model="searchForm">
        <el-form-item label="监护人姓名" label-width="100" class="el-form-item" prop="pName">
          <el-input v-model="searchForm.pName" placeholder="请输入要查询的父母姓名" />
        </el-form-item>
        <el-form-item label="孩子姓名" label-width="100" class="el-form-item" prop="sName">
          <el-input v-model="searchForm.sName" placeholder="请输入要查询的孩子姓名" />
        </el-form-item>
    ...
    <div class="bottom">
      ...
      <div class="el-table-inner">
        ...
        >
          ...
          <el-table-column prop="parentId" label="监护人ID" width="140"></el-table-column>
          <el-table-column prop="parentName" label="监护人姓名" width="100"></el-table-column>
          <el-table-column prop="sex" label="性别" width="60"></el-table-column>
          <el-table-column prop="studentId" label="孩子学号" width="150"></el-table-column>
          <el-table-column prop="studentName" label="孩子姓名" width="100"></el-table-column>
