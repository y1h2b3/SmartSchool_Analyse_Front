<script lang="ts" setup>
  ...
  import StudentInsertDialog from './studentInsertDialog.vue'
  import StudentUpdateDialog from './studentUpdateDialog.vue'
  ...
  import {
    ...
    deleteStudentInfo,
    updateStudentInfo,
  } from '@/api/admin/personnelManagement/student.ts'
  ...
  const column = [
    ...
    { name: 'teacherId', label: '班主任ID' },
    { name: 'teacherName', label: '班主任' },
    { name: 'parentId', label: '监护人ID' },
    { name: 'parentName', label: '监护人' },
  ...
  watch(
    () => props.currentChildrenNode,
    () => {
      if (currentChildrenNodeExists.value && isSuc.value > 2) {
        flushData(1)
      }
      isSuc.value++
    },
  ...
  const flushData = (index: number) => {
    ...
    setTimeout(async () => {
      ...
      const id = searchForm.studentId
      const name = searchForm.name
      const resp = await getPageStudentInfo(currentPage.value, pageSize.value, clazz, id, name)
  ...
  const handleBatchDelete = async () => {
    ...
    selectedRows.value.map(async (item) => {
      // 调用删除接口
      await deleteStudentInfo(item.studentId)
    })
    // 刷新数据
  ...
  const handleDelete = async (row) => {
    ...
    // 调用删除接口
    await deleteStudentInfo(row.studentId)
    // 刷新数据
    instance.emit('updateAllFreshData', !props.allFreshData.value)
    flushData(currentFresh.value)
    ElMessage.success('删除成功')
  ...
  const handleExportExcel = async () => {
    ...
    const id = searchForm.studentId
    const name = searchForm.name
    const pageResp = await getPageStudentInfo(currentPage.value, pageSize.value, clazz, id, name)
  ...
  <div class="content">
    ...
    <el-scrollbar class="top">
      <el-form class="el-form" :model="searchForm">
        <el-form-item label="学号" label-width="80" class="el-form-item" prop="studentId">
          <el-input v-model="searchForm.studentId" placeholder="请输入要查询的学号" />
        </el-form-item>
    ...
    <div class="bottom">
      ...
      <div class="el-table-inner">
        ...
        >
          ...
          <el-table-column prop="deviceId" label="设备号" width="110"></el-table-column>
          ...
          <el-table-column prop="teacherName" label="班主任" width="100"></el-table-column>
