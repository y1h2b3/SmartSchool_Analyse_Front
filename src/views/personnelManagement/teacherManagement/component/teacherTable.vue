<script lang="ts" setup>
  ...
  import TeacherInsertDialog from './teacherInsertDialog.vue'
  import TeacherUpdateDialog from './teacherUpdateDialog.vue'
  ...
  const column = [
    { name: 'order', label: '序号' },
    { name: 'teacherId', label: '工号' },
    { name: 'name', label: '姓名' },
    { name: 'sex', label: '性别' },
    { name: 'post', label: '岗位' },
    { name: 'clazz', label: '管理班级' },
    { name: 'status', label: '账号状态' },
    { name: 'phone', label: '手机号' },
    { name: 'password', label: '密码' },
    { name: 'birth', label: '出生日期' },
    { name: 'deviceId', label: '设备号' },
    { name: 'emergencyContact', label: '紧急联系人' },
  ...
  const flushData = (index: number) => {
    ...
    setTimeout(async () => {
      ...
      const id = ''
      const name = ''
      const resp = await getPageTeacherInfo(currentPage.value, pageSize.value, clazz, id, name)
  ...
  const handleBatchDelete = async () => {
    ...
    selectedRows.value.map(async (item) => {
      // 调用删除接口
      await deleteTeacherInfo(item.teacherId)
    })
    // 刷新数据
  ...
  const handleDelete = async (row) => {
    ...
    // 调用删除接口
    await deleteTeacherInfo(row.teacherId)
    // 刷新数据
    flushData(currentFresh.value)
    ElMessage.success('删除成功')
  }
  ...
  const handleExportExcel = async () => {
    ...
    const pageResp = await getPageTeacherInfo(currentPage.value, pageSize.value, clazz, id, name)
  ...
  <div class="content">
    ...
    <div class="top">
      <el-form class="el-form" :model="searchForm">
        <el-form-item label="工号" label-width="80" class="el-form-item" prop="studentId">
          <el-input v-model="searchForm.studentId" placeholder="请输入要查询的工号" />
        </el-form-item>
    ...
    <div class="bottom">
      ...
      <div class="el-table-inner">
        ...
        >
          ...
          <el-table-column prop="teacherId" label="工号" width="140"></el-table-column>
          ...
          <el-table-column prop="clazz" label="管理班级" width="100"></el-table-column>
