<script lang="ts" setup>
  ...
  import { getAllTeacherInfo, updateTeacherInfo } from '@/api/admin/personnelManagement/teacher.ts'
  ...
  watch(
    ...
    (val) => {
      ...
      teacherInfo.teacherId = val.teacherId
      teacherInfo.password = val.password
      teacherInfo.post = val.post
      teacherInfo.clazz = val.clazz
  ...
  const onConfirm = async () => {
    await teacherInfoFormRef.value.validate(async (validate) => {
      ...
      // 发送请求
      await updateTeacherInfo(teacherInfo)
      // 刷新数据
...
<template>
  <el-dialog v-model="dialogVisible" title="修改教师信息" width="50%" style="padding-right: 40px">
    <el-form
      class="el-form"
      :model="teacherInfo"
